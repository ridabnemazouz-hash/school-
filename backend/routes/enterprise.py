...
def restore_backup(backup_id: int, db: Session = Depends(get_db), owner=Depends(require_platform_owner)):
    backup = db.query(BackupRecord).filter(BackupRecord.id == backup_id).first()
    if not backup:
        raise HTTPException(status_code=404, detail="Backup not found")
    backup.status = "restoring"
    db.commit()
    return {"message": f"Restore initiated for {backup.filename}. Note: manual DB swap required in production.", "backup_id": backup_id}
...
def generate_report(body: dict, db: Session = Depends(get_db), owner=Depends(require_platform_owner)):
